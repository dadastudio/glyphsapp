#MenuTitle: Sync Subscript
# -*- coding: utf-8 -*-
__doc__="""

Syncs metrics and kerning groups of Subscript, and creates missing .subs glyphs
...
def syncSelected():
	...
	inferiors=("plus","minus","equal","parenleft","parenright")
	...
	for thisLayer in selectedLayers:
		...
		if thisLayer.parent.category=="Number" or sourceGlyphName in inferiors: # if selected glyph is a number
			
			suffixOffset = sourceGlyphName.find(".") # checks if has suffix eg .lf
			
			if suffixOffset>0:
				
				supName = sourceGlyphName[:suffixOffset]+"inferior"
			else:
				supName = sourceGlyphName+"inferior"
			kernName="inferior"			
		else:	
			...
			if ssPoz>0:
				supName= sourceGlyphName[:ssPoz]+".subs"+sourceGlyphName[ssPoz:]

			else:	
				supName=sourceGlyphName+".subs" 
			kernName=".subs"
